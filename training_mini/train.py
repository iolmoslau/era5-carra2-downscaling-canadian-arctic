...
def main(cfg: DictConfig) -> None:
    ...
    # [ERA5->CARRA2 adapter] Optional GPU input conditioner: bilinearly upsample the coarse
    # LR to the HR grid and append the static land-sea mask, as the first step after each
    # batch lands on device (before patching / the UNet). No-op for datasets that do not
    # define `make_conditioner`. `input_channels()` already reports the post-conditioning
    # channel count, so the model below is sized correctly. See datasets/era5_carra2.py.
    lr_conditioner = None
    if hasattr(dataset, "make_conditioner"):
        lr_conditioner = dataset.make_conditioner().to(dist.device)
        logger0.info("Enabled GPU LR conditioner (upsample + static invariant append)")
                ...
                with nvtx.annotate("Training iteration", color="green"):
                    ...
                    for n_i in range(num_accumulation_rounds):
                        with nvtx.annotate(
                            ...
                            # [ERA5->CARRA2 adapter] upsample coarse LR to HR grid + append
                            # static invariant, on GPU, before patching / the UNet.
                            if lr_conditioner is not None:
                                img_lr = lr_conditioner(img_lr)
                                if use_apex_gn:
                                    img_lr = img_lr.to(
                                        dtype=input_dtype
                                    ).to(memory_format=torch.channels_last)
                                else:
                                    img_lr = img_lr.to(input_dtype).contiguous()
                            loss_fn_kwargs = {
                                "net": model,
                                "img_clean": img_clean,
                                "img_lr": img_lr,
                        ...
                        if is_time_for_periodic_task(
                            ...
                            with torch.no_grad():
                                for _ in range(cfg.training.io.validation_steps):
                                    ...
                                    # [ERA5->CARRA2 adapter] see training loop above.
                                    if lr_conditioner is not None:
                                        img_lr_valid = lr_conditioner(img_lr_valid)
                                        if use_apex_gn:
                                            img_lr_valid = img_lr_valid.to(
                                                dtype=input_dtype
                                            ).to(memory_format=torch.channels_last)
