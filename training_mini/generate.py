...
def main(cfg: DictConfig) -> None:
    ...
    # [ERA5->CARRA2 adapter] optional GPU input conditioner (upsample coarse LR to HR grid
    # + append static invariant); no-op for datasets without `make_conditioner`. Applied to
    # each batch's LR below so both inference and the saved "input" group are at HR grid.
    lr_conditioner = None
    if hasattr(dataset, "make_conditioner"):
        lr_conditioner = dataset.make_conditioner().to(device)
        ...
        with torch_nvtx_profiler:
            ...
            for dataset_index, (image_tar, image_lr, *lead_time_label) in zip(
                ...
                if lr_conditioner is not None:
                    image_lr = lr_conditioner(image_lr)
